...
import { applyZoneFinish as applyZoneFinishMultiStrategy } from './spline-material-finish';
...
// Updated from actual Spline scene hierarchy (2024-11-20)
...
/**
 * Apply finish using multi-strategy approach
 * 
 * This function now uses the new material finish system with automatic fallback:
 * 1. Spline Variables (if configured)
 * 2. Material Cloning & Swapping
 * 3. Direct THREE.js manipulation
 * 
 * @deprecated Use applyZoneFinish from spline-material-finish.ts instead
...
export function applyZoneFinishDirect(
  ...
  // Use the new multi-strategy system
  const result = applyZoneFinishMultiStrategy(spline, zone, finish);
