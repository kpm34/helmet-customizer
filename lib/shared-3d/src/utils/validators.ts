 ...
 * @returns Validation result
 */
export function validateRgbColor(rgb: RGBColor): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  if (typeof rgb.r !== 'number') errors.push('RGB.r must be a number');
  if (typeof rgb.g !== 'number') errors.push('RGB.g must be a number');
  if (typeof rgb.b !== 'number') errors.push('RGB.b must be a number');
  ...
  if (!Number.isInteger(rgb.r)) warnings.push('RGB.r is not an integer. Will be rounded.');
  if (!Number.isInteger(rgb.g)) warnings.push('RGB.g is not an integer. Will be rounded.');
  if (!Number.isInteger(rgb.b)) warnings.push('RGB.b is not an integer. Will be rounded.');
 ...
 * @returns Validation result
 */
export function validateThreeColor(threeColor: number): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 * @returns Validation result
 */
export function validateColorSpec(colorSpec: ColorSpec): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 * @returns Validation result
 */
export function validateMaterialFinish(finish: MaterialFinish): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  if (finish.clearcoatRoughness !== undefined) {
    if (typeof finish.clearcoatRoughness !== 'number') {
      ...
      errors.push(
        `MaterialFinish.clearcoatRoughness out of range: ${finish.clearcoatRoughness}. Must be 0-1.`
      );
    }
  }
 ...
 * @returns Validation result
 */
export function validateMaterial(material: I3DMaterial): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
export function sanitizeRgb(rgb: RGBColor): RGBColor {
  return {
 ...
 */
export function sanitizeMaterialFinish(finish: MaterialFinish): MaterialFinish {
  const sanitized: MaterialFinish = {
    ...finish,
