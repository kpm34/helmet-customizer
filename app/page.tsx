...
export default function Home() {
  ...
  return (
    <main className="w-full h-screen overflow-hidden relative" suppressHydrationWarning>
      ...
      {/* Variable Test Panel */}
      {helmetLoaded && (
        <div className="fixed top-4 left-4 z-50 bg-black/80 p-4 rounded-lg shadow-lg backdrop-blur-sm">
          <h3 className="text-white font-bold mb-2">🧪 Variable Test (testVar)</h3>
          <div className="flex flex-col gap-2">
            <button
              onClick={() => {
                if (splineRef.current) {
                  const value = getVariable(splineRef.current, 'testVar');
                  console.log('📖 Current testVar:', value);
                  alert(`testVar = ${value}`);
                }
              }}
            ...
            <button
              onClick={() => {
                if (splineRef.current) {
                  setVariable(splineRef.current, 'testVar', 100);
                  console.log('✅ Set testVar = 100 (fully opaque)');
                }
              }}
            ...
            <button
              onClick={() => {
                if (splineRef.current) {
                  setVariable(splineRef.current, 'testVar', 50);
                  console.log('✅ Set testVar = 50 (half transparent)');
                }
              }}
            ...
            <button
              onClick={() => {
                if (splineRef.current) {
                  setVariable(splineRef.current, 'testVar', 0);
                  console.log('✅ Set testVar = 0 (fully transparent)');
                }
              }}
      ...
      {/* Customization Wizard Button */}
      {helmetLoaded && !showWizard && (
        <button
          onClick={() => setShowWizard(true)}
          className="fixed bottom-4 right-4 z-50 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-8 py-4 rounded-full shadow-2xl transition-all flex items-center gap-3 font-bold text-lg"
        >
          <span>🎨</span>
          <span>Open Customization Wizard</span>
        </button>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            <button
              ...
              className="absolute top-4 right-4 text-gray-500 hover:text-gray-700 text-2xl font-bold w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors"
            >
              ×
            </button>
            <CustomizationWizard />
      ...
      {/* Customization Panel */}
      {helmetLoaded && !showWizard && <CustomizationPanel />}
