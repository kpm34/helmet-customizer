...
export const useHelmetStore = create<HelmetState>((set, get) => ({
  ...
  setZoneColor: (zone, color) => {
    set((state) => {
      ...
      // Auto-sync hardware color with facemask (10% darker) and set to matte
      if (zone === 'facemask') {
        ...
        const darkerR = Math.round(r * 0.9);
        const darkerG = Math.round(g * 0.9);
        const darkerB = Math.round(b * 0.9);
